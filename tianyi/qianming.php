...
<?php @$qianming = intval(file_get_contents("../api/tongji/jiekou/qianming.dat")); ?>
<?php @$qianmingx = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/qianming.dat")); ?>
...
<title>QQ签名 - <?php echo $ming; ?>API</title>
...
<legend>QQ签名API</legend>
</fieldset>
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $qianming; ?>次·今日调用：<?php echo $qianmingx; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qianming.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qianming.php</span>
</p>
<p class="simpleTable">
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/qianming.php");
echo $result;
?></pre>
</div>
...
<pre class="layui-code"><a style="color:#fd4b5c;text-decoration:none；" target="_blank">QQ签名
$访问 http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qianming.php$</per>
</div>
</div>
</div>
</div>
