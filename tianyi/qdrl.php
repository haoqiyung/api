...
<?php @$qdrl = intval(file_get_contents("../api/tongji/jiekou/qdrl.dat")); ?>
<?php @$qdrlx = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/qdrl.dat")); ?>
...
<title>签到日历图生成 - <?php echo $ming; ?>API</title>
...
<legend>签到日历图生成API</legend>
</fieldset>
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $qdrl; ?>次·今日调用：<?php echo $qdrlx; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qdrl.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qdrl.php</span>
</p>
<p class="simpleTable">
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qdrl.php?Tick=24&Template=2&year=2021&month=8">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/qdrl.php?Tick=24&Template=2&year=2021&month=8</span>
</p>
...
<tbody>
<tr><td>Tick</td><td>是</td><td>要标记的日期以,划分 例如1,2,3</td></tr>
<tr><td>Template</td><td>是</td><td>签到图模板(目前可选1~10)</td></tr>
<tr><td>year</td><td>是</td><td>年份(用于生成指定的某年某月的日历,需要配合月份使用)</td></tr>
<tr><td>month</td><td>是</td><td>月份(用于生成指定的某年某月的日历,需要配合年份使用)</td></tr>
</tbody>
</table>
...
<pre class="layui-code"><img src="../api/qdrl.php?Tick=24&Template=2&year=2021&month=8" style="width: 45%;"></pre>
</div>
