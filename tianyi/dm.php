...
<?php @$dm = intval(file_get_contents("../api/tongji/jiekou/dm.dat")); ?>
<?php @$dmx = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/dm.dat")); ?>
...
<title>随机高清动漫图片 - <?php echo $ming; ?>API</title>
...
<legend>随机高清动漫图片API</legend>
</fieldset>
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $dm; ?>次·今日调用：<?php echo $dmx; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/dm.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/dm.php</span>
</p>
<p class="simpleTable">
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/dm.php");
echo $result;
?></pre>
</div>
...
<pre class="layui-code"><a style="color:#fd4b5c;text-decoration:none；" target="_blank">随机高清动漫图片
<?php echo $ming; ?>API随机高清动漫图片\n
━━━━━━━━━\n
用户:@%昵称%\n
━━━━━━━━━\n
±img=$访问 http://<?php echo $_SERVER['HTTP_HOST'];?>/api/dm.php$±\n
━━━━━━━━━\n
请求示例：http://<?php echo $_SERVER['HTTP_HOST'];?>/tianyi/dm.php\n
━━━━━━━━━\n
