...
<?php @$huang = intval(file_get_contents("../api/tongji/jiekou/huang.dat")); ?>
<?php @$huangx = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/huang.dat")); ?>
...
<title>黄历 - <?php echo $ming; ?>API</title>
...
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $huang; ?>次·今日调用：<?php echo $huangx; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/huang.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/huang.php</span>
</p>
<p class="simpleTable">
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/huang.php");
echo $result;
?></pre>
</div>
...
<pre class="layui-code"><a style="color:#fd4b5c;text-decoration:none；" target="_blank">黄历
$访问 http://<?php echo $_SERVER['HTTP_HOST'];?>/api/huang.php$</per>
</div>
</div>
</div>
</div>
