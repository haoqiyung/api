...
<?php @$saohua = intval(file_get_contents("../api/tongji/jiekou/saohua.dat")); ?>
<?php @$saohuax = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/saohua.dat")); ?>
...
<title>骚话 - <?php echo $ming; ?>API</title>
...
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $saohua; ?>次·今日调用：<?php echo $saohuax; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/saohua.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/saohua.php</span>
</p>
<p class="simpleTable">
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/saohua.php");
echo $result;
?></pre>
</div>
...
<pre class="layui-code"><a style="color:#fd4b5c;text-decoration:none；" target="_blank">骚话
$访问 http://<?php echo $_SERVER['HTTP_HOST'];?>/api/saohua.php$</per>
</div>
</div>
</div>
</div>
