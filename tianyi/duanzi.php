...
<?php @$duanzi = intval(file_get_contents("../api/tongji/jiekou/duanzi.dat")); ?>
<?php @$duanzix = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/duanzi.dat")); ?>
...
<title>段子 - <?php echo $ming; ?>API</title>
...
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $duanzi; ?>次·今日调用：<?php echo $duanzix; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/duanzi.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/duanzi.php</span>
</p>
<p class="simpleTable">
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/duanzi.php");
echo $result;
?></pre>
</div>
...
<pre class="layui-code"><a style="color:#fd4b5c;text-decoration:none；" target="_blank">段子
$访问 http://<?php echo $_SERVER['HTTP_HOST'];?>/api/duanzi.php$</per>
</div>
</div>
</div>
</div>
