...
<?php @$shijian = intval(file_get_contents("../api/tongji/jiekou/shijian.dat")); ?>
<?php @$shijianx = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/shijian.dat")); ?>
...
<title>当前时间 - <?php echo $ming; ?>API</title>
...
<legend>当前时间API</legend>
</fieldset>
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $shijian; ?>次·今日调用：<?php echo $shijianx; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/shijian.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/shijian.php</span>
</p>
<p class="simpleTable">
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/shijian.php");
echo $result;
?></pre></div>
...
<pre class="layui-code"><a style="color:#fd4b5c;text-decoration:none；" target="_blank">当前时间
$访问 http://<?php echo $_SERVER['HTTP_HOST'];?>/api/shijian.php$</per>
</div>
</div>
</div>
</div>
