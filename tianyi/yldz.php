...
<?php @$yldz = intval(file_get_contents("../api/tongji/jiekou/yldz.dat")); ?>
<?php @$yldzx = intval(file_get_contents("../api/tongji/jinri/".date("Y-m-d")."/yldz.dat")); ?>
...
<title>语录段子 - <?php echo $ming; ?>API</title>
...
<legend>语录段子API</legend>
</fieldset>
<blockquote class="layui-elem-quote">本接口总调用：<?php echo $yldz; ?>次·今日调用：<?php echo $yldzx; ?>次</blockquote>
</div>
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/yldz.php">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/yldz.php</span>
</p>
<p class="simpleTable">
...
<span class="url" data-clipboard-text="http://<?php echo $_SERVER['HTTP_HOST'];?>/api/yldz.php?city=<?php echo date("Y-m-d");?>">http://<?php echo $_SERVER['HTTP_HOST'];?>/api/yldz.php?city=<?php echo date("Y-m-d");?></span>
</p>
...
<tbody>
<tr><td>city</td><td>是</td><td>需要查询的日期(默认今日)</td></tr>
</tbody>
</table>
...
$result = file_get_contents("http://".$_SERVER['HTTP_HOST']."/api/yldz.php?city=".date("Y-m-d")."");
echo $result;
?></pre>
</div>
