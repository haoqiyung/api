...
function qrlogin()
{
    ...
    $url = 'https://ssl.ptlogin2.qq.com/ptqrlogin?u1=https%3A%2F%2Fvip.qq.com%2Floginsuccess.html&ptqrtoken=' . getqrtoken($_GET["sm_qrsig"]) . '&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=0-0-' . time() . '0000&js_ver=20010217&js_type=1&login_sig=&pt_uistyle=40&aid=549000912&daid=18&';
    ...
    if (preg_match("/ptuiCB\\('(.*?)'\\)/", $ret, $arr) && $_GET["sm_qrsig"]) {
        ...
        if ($r[0] == 0) {
            ...
            if ($pskey) {
                ...
                file_put_contents("log/vip/".$uin[1].".txt",$data);
                die($data);
            } else {
