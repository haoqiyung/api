...
function qrlogin()
{
    $url = 'https://ssl.ptlogin2.qq.com/ptqrlogin?u1=https%3A%2F%2Fqzs.qq.com%2Fqzone%2Fv5%2Floginsucc.html%3Fpara%3Dizone&ptqrtoken=' . getqrtoken($_GET["sm_qrsig"]) . '&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=0-0-' . time() . '0000&js_ver=10194&js_type=1&login_sig=' . $sig . '&pt_uistyle=40&aid=549000912&daid=5&';
    ...
    if (preg_match("/ptuiCB\\('(.*?)'\\)/", $ret, $arr) && $_GET["sm_qrsig"]) {
        ...
        if ($r[0] == 0) {
            ...
            if ($pskey) {
                ...
                file_put_contents("log/qzone/".$uin[1].".txt",$data);
                die($data);
            } else {
